...
class linear_mpc:
    def __init__(self, ω0_s,f0_c,Δω0_s,Δf0_c, noise):
        ...
        # GEKKO linear MPC
        m = GEKKO(remote=True) #True for Mac and Linux, False for Windows
        ...
        m.Equation([m.L + m.L.dt() == 0.0731915*(m.w_s) + -2.01078172 * (m.f_c) + 0.46147441])
...
def main(idx,run):
    file_name = 'benchmarks_multiple_runs_mpc_demand.csv'
    try:
    ...
    demand_list = [[50,60,70,80,90],
                   [100,110,120,130,140],
                   [170,180,190,200,210]]
    i = idx #demand scenario index

    w0s_c = ws0_list[i]
    f0_c_c = fc0_list[i]

    if i <= 2:
        dm = demand_list[0][run]
    elif i > 2 and i <= 5:
        dm = demand_list[1][run]
    elif i > 5 and i <= 8:
        dm = demand_list[2][run]
    ...
    extruder_sim.reset(noise = 0.0001,
        ...
        demand = dm, #50 to 500
        cost = 0.86, #0.86 to 1.6
        selling_price = 2.0, #1.8 to 3.2
    ...
    mpc_L_l = []
    mpc_profit_l = []
    model_name = f'linear_mpc_{w0s_c}_{f0_c_c}_{dm}'
